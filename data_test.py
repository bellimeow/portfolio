...
class DataTest(unittest.TestCase):
    ...
    def test_load(self):
        ...
        #Dessa test la student micab709 till
        self.assertEqual(data.load(["Hello"]), None)
        #slut
    ...
    def test_search(self):
        ...
        #Dessa test la student micab709 till
        res = data.search(self.loaded_data, sort_order = 'desc', techniques=["python"])
        self.assertEqual(res[0]['start_date'], '2009-09-08')
        self.assertEqual(res[1]['start_date'], '2009-09-07')
        self.assertEqual(res[2]['start_date'], '2009-09-05')

        res = data.search(self.loaded_data, sort_by = 'course_name', sort_order = 'asc')
        self.assertEqual(res[0]['course_name'], 'HOHO')
        self.assertEqual(res[1]['course_name'], 'OKÄNT')
        ...
        self.assertEqual(res[1]['project_id'], 3)
        self.assertEqual(res[2]['project_id'], 2)
        # Slut

        # Assertar att alla fält kollas ifall "search_fields" är None
        res = data.search(self.loaded_data, search="okänt")
        self.assertEqual(len(res), 3)
