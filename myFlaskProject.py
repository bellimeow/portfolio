...
def website_index():
    ...
    #Check to see if the database with projects is longer than five projects. The index website displays the five newest projects sorted by project ID.
    if len(db)> 5:
        db = db[-5:]
    db.reverse()
...
def project_techniques():
    ...
    # Get the selected values from the technique page and split each search value into a list.
    ...
    technique_list = [{'label': technique, 'selected': technique in selected_techniques_list}
...
def show_project(id = None):
    ...
    # Checking if image file has a valid address. Might want to get a check to see that the file exists instead
...
def get_projects_by_selected_techniques(selected_techniques):
    ''' Returns all projects from the default json database that used all techiques in the passed list of techniques. If the passed list is None all projects will be returned.'''
    db = get_data()
...
def assign_random_wing_pair_to_technique_list(technique_list):
    '''Iterates a list of technique dictionaries and adds a set of matching characters to each dictionary with the keys 'left_wing' and 'right_wing'.'''
