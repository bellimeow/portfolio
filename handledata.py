...
def load(filename):
    '''Loads a file and returns a list of all the projects sorted by project id:s.'''
    try:
...
def get_technique_stats(db):
    '''Returns a dictionary containing techniques and the projects in which they are used. The projects are represented only by their id and name.'''
    technique_stats = {}

    for project in db:
        # For each project we insert information about the project's id and name.
...
def search(db, sort_by = 'start_date', sort_order = 'desc', techniques = None, search = None, search_fields = None):
    '''Returns a list of projects that have a field that contains the specified user input and sort by any field, either ascending or descending. Defaults to sorting by start date. If search_fields is None, then all the projects fields are searched and returned.'''
    matching_projects = []
...
def search_in_project(project, techniques, search, search_fields):
    ...
    if isinstance(search_fields, list) and len(search_fields) == 0:
        # If list of search fields are empty (but not None!), no search can be done
        return False
    elif not search_fields:
        return recursive_search_object(project, search) 
    else:
        # Check for search in each of the search fields. If any of fields contain the search string, return True.
...
def recursive_search_object(obj, search):
    '''Recursively searches any object, if object is a container then it's children will be searched instead.'''
    ...
    # If the object happens to be an integer or other non-string type, then convert it to a string and search it.
    return search in str(obj).lower()



def sort_projects(db, sort_by, sort_order):
    '''Returns a list of all the projects sorted by a specified field and in a specified order.'''

    sort_descending = sort_order == 'desc'

    # '\uffff' is default key when sorted by ascending. \uffff represents the highest possible unicode character.
    # '\u0000' is default key when sorted by descending. \u0000 represents the lowest possible unicode character.
    missing_key = '\u0000' if sort_descending else '\uffff'

    # If the search field is not present in the project then always sort it last.
